if { ![project_exists {fir_top}] } {
     project_new {fir_top}
} else {
     project_open {fir_top}
}
set_project_settings -cmp {fir_top}
if { ![project_settings_exist -sim fir_top] } {
        set_project_settings -sim fir_top
}
set_global_assignment -section_id {fir_top} -name {EDA_DESIGN_ENTRY_SYNTHESIS_TOOL} {PRECISION SYNTHESIS}
set_global_assignment -section_id {fir_top} -name {EDA_SIMULATION_TOOL} {MODELSIM (VHDL OUTPUT FROM QUARTUS II)}
set_global_assignment -name {EDIF_FILE} {fir_top.edf}
...
set_global_assignment -section_id {fir_top} -name {DEVICE} {EP2C35F672C7}
set_global_assignment -name {FAMILY} {CYCLONE II}
set_global_assignment -name {PROJECT_SHOW_ENTITY_NAME} {OFF}
if { [file exists {fir_top_pnr_constraints.tcl}] } {
  source fir_top_pnr_constraints.tcl
}
